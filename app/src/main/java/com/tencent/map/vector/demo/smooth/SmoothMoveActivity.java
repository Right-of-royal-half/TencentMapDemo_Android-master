    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_set:
                //添加小车路线
                map.addPolyline(new PolylineOptions().add(mCarLatLngArray));
                break;
            case R.id.satrt:
                //添加小车
                ...
                mCarMarker = map.addMarker(
                        new MarkerOptions(carLatLng)
                                .anchor(0.5f, 0.5f)
                                .icon(BitmapDescriptorFactory.fromBitmap(getBitMap(R.drawable.navi_marker_location)))
                                .flat(true)
                                .clockwise(false));
                //创建移动动画
                ...
                Log.d("TAG", "onClick: " + mAnimator.getAnimatorSet());
                break;
        }
    }
